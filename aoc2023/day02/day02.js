...
console.log((require('fs').readFileSync('input.txt')+"")[s='split']`\n`.map(a=>a[s]`:`[1][s]`;`.map(b=>[b,b[x='indexOf']('d')-5,b[x]('g')-3,b[x]('b')-3]).map(c=>c.slice(1).map(d=>d>=0?+c[0].slice(d,d+3):0))).map(e=>eval([0,0,0].map((_,i)=>Math.max(...[...e,[0,0,0]].map(r=>r[i]))).join`*`)).reduce((a,b)=>a+b,0))
